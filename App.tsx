                  ...
                  <div className="relative w-full h-32">
                      ...
                      <div className="absolute right-3 bottom-3 z-10 flex items-center gap-2">
                           <Button onClick={isEditingLoading ? handleStopEdit : handleEditGenerate} isLoading={isEditingLoading} variant={isEditingLoading ? "rainbow-stop" : "rainbow"} className="py-2 px-6 text-xs font-bold flex items-center gap-2 min-w-[120px] hover:scale-105 active:scale-95 transition-transform duration-200">
                                {isEditingLoading ? (<><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" className="mr-2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>STOP</>) : (<><svg width="16" height="16" viewBox="0 0 100 100" className="fill-current text-white mr-1"><path d="M 50 0 C 50 35 60 45 100 50 C 60 55 50 65 50 100 C 50 65 40 55 0 50 C 40 45 50 35 50 0 Z" /></svg>Generate</>)}
                            </Button>
                      </div>
                  </div>
             </GlassCard>
...
interface AppProps {
  user: User; // Changed from onSignOut
  onSignOut: () => void;
}
...
const App: React.FC<AppProps> = ({ user, onSignOut }) => {
  ...
  // Use state to track current credits locally for immediate UI update
  ...
  // Dropdown Refs & States (Omitted for brevity as they remain unchanged)
  // ... (Keep existing refs and dropdown states logic)
  ...
  useEffect(() => {
      if (!isRenderEngineDropdownOpen) setLockedRenderCategory(null);
  }, [isRenderEngineDropdownOpen]);
  useEffect(() => {
      if (!isLightingDropdownOpen) setLockedLightingCategory(null);
  }, [isLightingDropdownOpen]);
  useEffect(() => {
      if (!isArchDropdownOpen) setLockedArchCategory(null);
  }, [isArchDropdownOpen]);
  ...
  // ... (Keep existing loading bar effect)
   useEffect(() => {
  ...
  const handleTriggerEdit = async (maskBase64: string, editPrompt: string, references: string[]) => {
      ...
      if (currentUser.credits < 1) {
          setError("Insufficient credits to perform edit.");
          return;
      }
      ...
      try {
          ...
          // 1. Transaction Log (Deduct 1 credit)
          const tx = await apiService.logTransaction(currentUser.username, "Image Edit", -1);
          ...
          // 2. Upload Result to Drive (Edit Mode)
          apiService.uploadGeneratedImage(currentUser.username, editedUrl, editPrompt, true);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // --- LOG TRANSACTION FOR BATCH ---
      // We deduct 1 credit per image generated (or -1 per batch? User said 1 image is 1 credit)
      // Since `generateCreativeAsset` returns multiple if `effectiveCount` > 1.
      // We will deduct `imageUrls.length`.
      const totalCost = -1 * imageUrls.length;
      const tx = await apiService.logTransaction(currentUser.username, `Generate ${imageUrls.length} Image(s)`, totalCost);
      ...
      imageUrls.forEach(url => {
          apiService.uploadGeneratedImage(currentUser.username, url, prompt, false);
      });
  ...
  const handleValidateAccess = (): boolean => true;
    ...
    <div className="min-h-screen w-full relative bg-[#0f172a] selection:bg-white selection:text-[#0f172a] flex flex-col">
      ...
      <header className="flex-none h-24 flex items-center justify-center mb-2 z-20 select-none">
          <div className="w-full max-w-[1920px] mx-auto px-6 md:px-12 lg:px-20 xl:px-28 flex items-center justify-between gap-3">
              ...
              {/* USER PROFILE SECTION - Pass user state and onSignOut */}
              <UserProfile user={currentUser} onSignOut={onSignOut} />
          </div>
      </header>
      
      {/* ... (Keep Main Container and Footer structure same as existing code) ... */}
      <div className="flex-1 w-full max-w-[1920px] mx-auto px-6 md:px-12 lg:px-20 xl:px-28 flex flex-col lg:flex-row gap-16 relative z-10 items-stretch min-h-[calc(100vh-7rem)] pb-24">
          ...
          <GlassCard className="w-full lg:w-[360px] xl:w-[420px] shrink-0 flex flex-col p-5 lg:p-6 h-full relative z-50">
             {/* ... Keep all existing dropdowns and inputs (Graphic Mode, Arch Mode, Prompt Area, etc.) ... */}
                   ...
                   <div className="relative" ref={archDropdownRef}>
                    ...
                    {isArchDropdownOpen && (
                      <div className="absolute top-full left-0 w-full mt-1 z-50 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.3)] p-1 overflow-visible">
                        <button onClick={() => handleArchSelect(ArchitectureStyle.STANDARD)} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm mb-1 ${selectedArchStyle === ArchitectureStyle.STANDARD ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className="font-medium">Standard</span></button>
                        {Object.entries(ARCH_STYLE_GROUPS).map(([category, styles]) => {
                            ...
                            <div key={category} className="group/item relative">
                                <button onClick={(e) => { e.stopPropagation(); setLockedArchCategory(isActive ? null : category); }} className={`w-full p-2 rounded flex items-center justify-between gap-2 text-left text-sm transition-colors ${isActive ? 'bg-white/10 text-white' : 'text-white/80 hover:bg-white/10'}`}><span className="font-medium flex-1">{category}</span><ChevronRight size={14} className={`transition-transform duration-200 opacity-50 flex-none ${isActive ? 'rotate-90' : ''}`} /></button>
                                <div className={`absolute left-full top-0 ml-2 w-48 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-1 z-[60] ${isActive ? 'block' : 'hidden'}`}>{styles.map((style) => (<button key={style} onClick={() => handleArchSelect(style)} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm ${selectedArchStyle === style ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className={`w-1.5 h-1.5 rounded-full flex-none ${selectedArchStyle === style ? 'bg-white' : 'bg-white/20'}`} /><span className="font-medium flex-1">{style}</span></button>))}</div>
                            </div>
                        )})}
                        <button onClick={() => handleArchSelect(ArchitectureStyle.OTHERS)} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm mt-1 ${selectedArchStyle === ArchitectureStyle.OTHERS ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className="font-medium">Others</span></button>
                      </div>
                    )}
                   </div>
             </div>
             ...
             <div className="grid grid-cols-3 gap-3 mb-6 flex-none">
                  {/* Keep Ratio, Count, Quality logic exactly as before */}
                  <div className="relative" ref={ratioDropdownRef}>
                     ...
                     <button onClick={() => setIsRatioDropdownOpen(!isRatioDropdownOpen)} className={`w-full p-3 rounded-lg flex items-center justify-between border transition-all ${isRatioActive ? activeButtonStyle : inactiveButtonStyle}`}><div className="flex items-center gap-2 truncate"><span className="truncate text-sm font-medium">{isRatioActive ? ASPECT_RATIOS.find(r => r.value === selectedAspectRatio)?.label : 'Select'}</span></div><ChevronDown size={14} className={isRatioActive ? 'text-white' : 'text-white/30'} /></button>
                      {isRatioDropdownOpen && (
                        <div className="absolute top-full left-0 w-full mt-1 z-50 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden p-1 min-w-[120px]">{ASPECT_RATIOS.map((ratio) => (<button key={ratio.value} onClick={() => { setSelectedAspectRatio(ratio.value); setIsRatioAuto(false); setIsRatioDropdownOpen(false); }} className={`w-full p-2 rounded flex items-center justify-between gap-2 text-left text-sm ${selectedAspectRatio === ratio.value ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><div className="flex items-center gap-2"><span>{ratio.label}</span></div></button>))}</div>
                      )}
                  </div>
             ...
             {/* ... (Existing Engine/Lighting Logic) ... */}
             {isArchModeActive && (
                    <div className="grid grid-cols-2 gap-3 mb-6 flex-none animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="relative" ref={renderEngineDropdownRef}>
                            ...
                            {isRenderEngineDropdownOpen && (
                                <div className="absolute top-full left-0 w-full mt-1 z-50 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.3)] p-1 overflow-visible">
                                    <button onClick={() => { setSelectedRenderEngine(RenderEngine.DEFAULT); setIsRenderEngineDropdownOpen(false); }} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm mb-1 ${selectedRenderEngine === RenderEngine.DEFAULT ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className="font-medium">Default</span></button>
                        ...
                        <div className="relative" ref={lightingDropdownRef}>
                            ...
                            {isLightingDropdownOpen && (
                                <div className="absolute top-full left-0 w-full mt-1 z-50 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.3)] p-1 overflow-visible">
                                    <button onClick={() => { setSelectedLighting(LightingSetting.DEFAULT); setIsLightingDropdownOpen(false); }} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm mb-1 ${selectedLighting === LightingSetting.DEFAULT ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className="font-medium">Default</span></button>
                                    {Object.entries(LIGHTING_GROUPS).map(([category, options]) => {
                                        ...
                                        <div key={category} className="group/item relative">
                                            ...
                                            <div className={`absolute left-full top-0 ml-2 w-48 bg-[#0f172a]/95 backdrop-blur-xl border border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-1 z-[60] ${isActive ? 'block' : 'hidden'}`}>{options.map((option) => (<button key={option} onClick={() => { setSelectedLighting(option); setIsLightingDropdownOpen(false); setLockedLightingCategory(null); }} className={`w-full p-2 rounded flex items-center gap-2 text-left text-sm ${selectedLighting === option ? 'bg-white/20 text-white font-bold' : 'text-white/60 hover:bg-white/10'}`}><span className="font-medium flex-1">{option}</span></button>))}</div>
                                        </div>
                                    )})}
                                </div>
                            )}
             ...
             {/* ... (Existing Image Upload Areas & Prompt) ... */}
                      ...
                      <div className="w-full flex justify-center">
                         ...
                         >
                            {loading ? (<><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" className="mr-2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>STOP</>) : (<>
                                <svg width="20" height="20" viewBox="0 0 100 100" className="w-5 h-5 mr-2 text-white fill-current"><path d="M 50 0 C 50 35 60 45 100 50 C 60 55 50 65 50 100 C 50 65 40 55 0 50 C 40 45 50 35 50 0 Z" /></svg>
          ...
          <div className="w-full lg:flex-1 h-auto flex flex-col gap-6 min-w-0 relative z-10">
              <GlassCard className="flex-1 w-full flex flex-col relative overflow-hidden min-h-[400px] shrink-0 rounded-2xl">
                  {/* Metal Corners */}
                  <div className="absolute -top-[1px] -left-[1px] z-20 pointer-events-none mix-blend-plus-lighter opacity-80"><svg width="120" height="120" viewBox="0 0 120 120" fill="none"><defs><radialGradient id="gradTL" cx="0" cy="0" r="120" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#ffffff" stopOpacity="0.9" /><stop offset="20%" stopColor="#94a3b8" stopOpacity="0.6" /><stop offset="100%" stopColor="#ffffff" stopOpacity="0" /></radialGradient></defs><path d="M 1 118 V 20 Q 1 1 20 1 H 118" stroke="url(#gradTL)" strokeWidth="1.5" strokeLinecap="round" fill="none"/></svg></div>
                  <div className="absolute -bottom-[1px] -right-[1px] z-20 pointer-events-none mix-blend-plus-lighter opacity-80"><svg width="120" height="120" viewBox="0 0 120 120" fill="none"><defs><radialGradient id="gradBR" cx="120" cy="120" r="120" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#ffffff" stopOpacity="0.9" /><stop offset="20%" stopColor="#94a3b8" stopOpacity="0.6" /><stop offset="100%" stopColor="#ffffff" stopOpacity="0" /></radialGradient></defs><path d="M 119 2 V 100 Q 119 119 100 119 H 2" stroke="url(#gradBR)" strokeWidth="1.5" strokeLinecap="round" fill="none"/></svg></div>
                        ...
                        <div className={`w-full h-full p-2 grid gap-4 relative z-20 ${ (loading ? imageCount : generatedImages.length) === 1 ? 'grid-cols-1' : (loading ? imageCount : generatedImages.length) === 2 ? 'grid-cols-2' : 'grid-cols-2 grid-rows-2' }`}>
                           {Array.from({ length: loading ? imageCount : generatedImages.length }).map((_, i) => {
                             ...
                             if (completedUrl) {
                               ...
                               return (
                                 <div key={`load-${i}`} className="relative bg-[#0f172a]/40 backdrop-blur-md rounded-2xl border border-white/20 flex items-center justify-center overflow-hidden">
                                     {/* AILoader Code inline or imported */}
                                     <div className={`flex flex-col items-center justify-center gap-3 z-10 ${imageCount > 1 ? 'scale-75' : 'scale-100'}`}>
                                        <div className="relative flex items-center justify-center">
                                            ...
                                            <svg width="80" height="80" viewBox="0 0 100 100" className="animate-pulse drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" xmlns="http://www.w3.org/2000/svg">
                                                <defs><linearGradient id="starGradient" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#1e40af" /><stop offset="100%" stopColor="#ffffff" /></linearGradient></defs>
                                        ...
                                        <div className="text-center mt-3"><span className="block text-white font-mono font-bold text-2xl leading-none">{Math.round(progress)}%</span></div>
                                    </div>
                                 </div>
                               );
                             }
              ...
              <GlassCard className="w-full flex-none h-40 min-h-[10rem] shrink-0 p-4 flex flex-col mb-8 lg:mb-0">
                  ...
                  <div className="flex-1 flex gap-4 overflow-x-auto custom-scrollbar pb-2 min-h-0">
                    {history.length === 0 ? (<div className="w-full flex items-center justify-center text-white/30 text-xs italic">No history yet. Start creating!</div>) : (
                        history.map((item) => (
                        <div key={item.id} onClick={() => { setGeneratedImages([item.url]); setLoading(false); setIsLastGenEdit(item.isEdit || false); }} className="relative flex-none h-full aspect-square rounded-lg overflow-hidden border border-white/20 group hover:border-white transition-all cursor-pointer select-none" onContextMenu={(e) => e.preventDefault()}>
